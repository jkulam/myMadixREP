...
<?php require_once "table_top.php";?>
 <div id='table_today'>
                             <table  class="table table-striped table-bordered" id="example"  alt="Sales_orders" style="display:block; margin:0px auto;position:relative;" >
...
?>
							...
							<?php }?>
                          <div id='example_num' style="display:none;" alt='datastore'>5000</div>
...
// alert($.trim(item.Type)+'=='+$.trim(shipk.Tknum) +'&&'+ $.trim(item.Seqnr)+'!=901');
			   ...
			   if($.trim(shipk.TRACKURl)!='')
				{
				   bol_ship += '<td style="width:50px"><a href="'+shipk.TRACKURl+'" target="_blank" ><img src="../images/s_b_detl.gif"/></a></td>';
			   }
			   else
				{
				   bol_ship += '<td style="width:50px">N/A</td>';
				   
				}
				   bol_ship += '<td style="width:50px">'+shipk.TEXT1+'</td></tr>';
...
if(!$.isArray(json_data.TSHIPMENTP))
	...
	if($.trim(json_data.TSHIPMENTK[0].TKNUM)==$.trim(json_data.TSHIPMENTP.TKNUM))
				{
			$('#he1').html('| Shipment of order K'+json_data.TSHIPMENTP.VGBEL)
